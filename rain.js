...

{
    ...
    function gettod_bgcolor() {
        ...
        if (nextIndex === -1) return `rgb(${palette[palette.length - 1].color.join(',')})`;
        ...
        const r = Math.round(prev.color[0] + (next.color[0] - prev.color[0]) * progress);
        const g = Math.round(prev.color[1] + (next.color[1] - prev.color[1]) * progress);
        const b = Math.round(prev.color[2] + (next.color[2] - prev.color[2]) * progress);
